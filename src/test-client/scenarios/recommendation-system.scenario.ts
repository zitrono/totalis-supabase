...
export async function runRecommendationSystemScenario(
  ...
  try {
    ...
    const activeRecsResponse = await supabase.functions.invoke('recommendations', {
      body: {
        type: 'active',
        limit: 5
      }
    ...
    if (categories.length > 0) {
      ...
      const checkinRecsResponse = await supabase.functions.invoke('recommendations', {
        body: {
          type: 'checkin',
          checkinId: checkInId
        }
    ...
    if (categories.length > 0) {
      const categoryRecsResponse = await supabase.functions.invoke('recommendations', {
        body: {
          type: 'category',
    ...
    const updatedRecsResponse = await supabase.functions.invoke('recommendations', {
      body: {
        type: 'active',
        limit: 10
      }
    ...
    const paginatedResponse = await supabase.functions.invoke('recommendations', {
      body: {
        type: 'all',
        limit: 2,
        offset: 0
    ...
    const { data: viewedRecs } = await supabase
