  ...
  try {
    ...
    checkIns?.forEach(checkIn => {
      if (checkIn.categories) {
        ...
        if (new Date(checkIn.created_at) > new Date(categoryStats[catId].lastUsed)) {
    ...
    if (recommendationsCount && recommendationsCount > 0) {
      summary.insights.push(`You've received ${recommendationsCount} personalized recommendations this ${period}.`)
    }
    ...
    return new Response(
      JSON.stringify({
        summary,
        details: {
          checkInsPerDay: (totalCheckIns / Math.max(1, Math.ceil((endDate.getTime() - startDate.getTime()) / (1000 * 60 * 60 * 24)))).toFixed(1),
          completionRate: totalCheckIns > 0 ? ((completedCheckIns / totalCheckIns) * 100).toFixed(0) + '%' : '0%',
