  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      if (attempt < maxRetries) {
        ...
        console.log(`Transcription attempt ${attempt} failed, retrying after ${delay}ms...`)
        await new Promise(resolve => setTimeout(resolve, delay))
      }
    }
  }
...
async function transcribeAudio(options: {
  ...
  const { audio_url, audio_base64, language = 'en', prompt, supabaseServiceKey, openaiApiKey } = options
  ...
  if (audioBlob.size > maxSize) {
    throw new Error(`Audio file too large. Maximum size is 25MB, got ${(audioBlob.size / 1024 / 1024).toFixed(2)}MB`)
  }
...
serve(async (req) => {
  ...
  try {
    ...
    const { audio_url, audio_base64, language = 'en', prompt, message_id, batch } = body
