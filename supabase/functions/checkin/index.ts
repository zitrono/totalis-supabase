...
serve(async (req) => {
  ...
  try {
    ...
    switch (action) {
      case 'start':
        result = await startCheckin(supabase, user.id, body, testMetadata, monitoring)
        break
      ...
      default:
        throw new Error('Invalid action. Must be one of: start, answer, complete, abandon')
    }
...
async function startCheckin(
  ...
  if (existingCheckin) {
    ...
    const questions = existingCheckin.metadata?.template_questions || generateInitialQuestions(category, [])
...
async function submitAnswer(supabase: any, userId: string, body: CheckinRequest) {
  ...
  if (!checkinId || !questionId || answer === undefined) {
    throw new Error('checkinId/checkin_id, questionId/question_id, and answer are required')
  }
...
function generateInitialQuestions(category: any, recentCheckins: any[]): CheckinQuestion[] {
  // Generate unique question IDs
  const createQuestionId = (index: number) => `q_${category.id}_${Date.now()}_${index}`
    ...
    'Physical Health': [
      {
        ...
        text: "How would you rate your physical energy today on a scale of 1-10?",
        type: 'scale',
        min: 1,
        max: 10,
        required: true
      ...
      questions = [
        {
          ...
          text: "I noticed things were challenging last time. How are you feeling now?",
          type: 'text',
          required: true
        },
        ...questions
