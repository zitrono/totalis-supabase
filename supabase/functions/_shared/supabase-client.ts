...
export async function getUserContext(supabase: any, userId: string): Promise<UserContext> {
  // Get user's coach
  ...
  const recentCategories = [...new Set(recentCheckIns?.map((c: any) => c.category_id) || [])] as string[]
