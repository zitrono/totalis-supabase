    ...
    if (pathname.endsWith("/api/user/recommendation/get")) {
      ...
      if (data && data.length > 0) {
        result = data.map((rec, index) =>
          transformToLegacyFormat(rec, index + 1)
        );
      } else {
      ...
      if (data && data.length > 0) {
        ...
        result = checkinIds.flatMap((id: string) =>
          generateMockRecommendations(id, 1)
        );
      }
      ...
      if (data && data.length > 0) {
        result = data.map((rec, index) =>
          transformToLegacyFormat(rec, index + 1, true)
        );
      } else {
        result = checkinIds.flatMap((id: string) =>
          generateMockRecommendations(id, 2, true)
        );
      }
    } else {
      return new Response(
...
function transformToLegacyFormat(
  ...
  return {
    ...
    icon_id: rec.category_id
      ? Math.abs(rec.category_id.charCodeAt(0)) % 10 + 1
      : 1,
    order: order,
...
function generateMockRecommendations(
  ...
  for (let i = 0; i < count; i++) {
    recommendations.push({
      ...
      why:
        "Research shows this approach is effective for building positive habits",
      action: isSecond
